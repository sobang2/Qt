        ...
        : AbstractMeter(parent)
{
    ...
    m_maximum=100.0; // First we set manualy to call calcMaxMin
  	setMinimum(0.0);
...
void ManoMeter::paintBackground(QPainter & painter)
{
    ...
    QSize Size =  painter.fontMetrics().size(Qt::TextSingleLine, m_speedname);
    if(m_niddleColor == "darkgreen")
        painter.setPen(Qt::darkGreen);
    else if(m_niddleColor == "pink")
        painter.setPen(Qt::magenta);
    painter.drawText(QPointF(Size.width()*-.5,height()*0.5), m_speedname);
...
//          painter.drawText( QPointF( Size.width() / -2.0,static_cast<int>( 0 - valueOffset())) , Str);
          painter.drawText( QPointF( Size.width() / -2.0,static_cast<int>( 0)) , Str);
          painter.setPen(Qt::white);
          painter.drawText(QPointF(-20,Size.height()),suffix());
        }
}// paintEvent
